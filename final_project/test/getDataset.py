...
def get_fridge_dateset():
    ...
    # data['timestamp'] = pd.to_datetime(data['Date'] + ' ' + data['Time'], errors='coerce', dayfirst=True)
    ...
    # features = ['Fridge_Temperature', 'Temp_Condition', 'timestamp']
    X = data[features]
    y = data['label']
    # print(data[['Fridge_Temperature', 'Temp_Condition', 'timestamp','label']].head(5))
